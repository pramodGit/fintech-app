...
A modern Angular application demonstrating the **full spectrum of Agentic AI** — from a polished UI simulation to a real LLM-powered agent loop using Claude claude-sonnet-4-20250514 with live tool calls.
...
| `/real-agent` | Real Agentic AI | Claude API · `tool_use` loop · live reasoning trace |
...
Claude reasons → next tool or final answer
...
| LLM | Anthropic Claude claude-sonnet-4-20250514 via `/v1/messages` |
...
- **Real agent loop** — multi-turn `tool_use` → `tool_result` cycle with Claude, visible in the tool call inspector
